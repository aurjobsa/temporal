...
class ParallelProcessingWorkflow:
    ...
    async def _execute_screening_step(
        ...
        try:
            await self._notify(
                chat_id, "orchestrator_update",
                {"functionName": "Screening", "status": "started"},
            )
    ...
    async def _execute_voice_calls_step(
        ...
        try:
            await self._notify(
                chat_id, "orchestrator_update",
                {"functionName": "trigger_voice_call", "status": "started"},
            )
            ...
            # Phase 2: wait for webhook signal per call SID
            completed_calls = []
            for call_sid in call_sids:
                try:
                    ...
                    completed_calls.append(self.call_completions[call_sid])
                    workflow.logger.info(f"Call completed: sid={call_sid}")
                except Exception:
                    completed_calls.append({"call_status": "timeout"})
                    workflow.logger.warning(f"Call timed out: sid={call_sid}")

                # Persist individual call result
                await workflow.execute_activity(
                    save_voice_call_result,
                    args=[chat_id, tenant_id, call_sid, completed_calls[-1]],
                    start_to_close_timeout=_SHORT_TIMEOUT,
                    retry_policy=_RETRY_POLICY,
                )

            successful_calls = sum(
                1 for c in completed_calls if c.get("call_status") == "completed"
            )
            failed_calls = len(call_results) - successful_calls

            call_details = [
                {
                    "call_sid":      call_sids[i] if i < len(call_sids) else None,
                    "call_status":   c.get("call_status", "unknown"),
                    "transcript":    c.get("transcript", []),
                    "call_duration": c.get("call_duration", 0),
                }
                for i, c in enumerate(completed_calls)
            ]
    ...
    async def _execute_emails_step(
        ...
        try:
            await self._notify(
                chat_id, "orchestrator_update",
                {"functionName": "Sending_final_emails", "status": "started"},
            )
